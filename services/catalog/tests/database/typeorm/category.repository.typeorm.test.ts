...
describe("TypeORMCategoryRepository", () => {
  ...
  it("should find highlighted categories", async () => {
    ...
    const highlightedCategories =
      await categoryRepository.findHighlightedCategories();
    ...
    expect(highlightedCategories[0].products![0].name).toBe(
  ...
  it("should find category by uuid", async () => {
    ...
    // When
    const foundCategory = await categoryRepository.findByUuid(
      categoryEntity.uuid,
    );
    ...
    // DFD
    expect(foundCategory!.products![0].name).toBe(
      foundCategory!.products![0].name,
    );
  });
