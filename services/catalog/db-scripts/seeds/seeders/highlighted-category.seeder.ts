...
export async function seedHighlightedCategories(
  highlightedCategoryRepo: Repository<HighlightedCategoryEntity>,
  categories: CategoryEntity[],
) {
  const highlightedNames = ["Festival", "Silk Dresses", "Suits", "Showroom"];

  const highlightedCategories = categories.filter((category) =>
    highlightedNames.includes(category.name),
  );
