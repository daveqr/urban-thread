...
const connectDB = async () => {
  try {
    await mongoose.connect("mongodb://mongodb:27017/apparel", {
...
const main = async () => {
  ...
  try {
    ...
    await insertCategory('Silk Dresses', editions[4]._id, 'Collection of silk dresses for Pre-fall season.', products.slice(0, 1));
    await insertCategory('Suits', editions[3]._id, 'Collection of designer suits.', products.slice(0, 2));
    await insertCategory('Festival', editions[2]._id, 'Collection of suits for the summer season.', products.slice(2, 5));
    await insertCategory('Showroom', editions[0]._id, 'Collection of clothing on sale in our showroom.', products.slice(4, 8));
  } catch (error) {
    console.error('Error connecting to MongoDB:', error);
  } finally {
